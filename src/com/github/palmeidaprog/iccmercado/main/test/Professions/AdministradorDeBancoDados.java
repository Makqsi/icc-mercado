...
public class AdministradorDeBancoDados implements Professionable, Linkable {
    // Links
    private URL youtube;
    ...
    private final String AREA = "Banco de Dados (Big Data)";
    private final String PROFESSION = "Administrador de Banco de Dados";


    //--Singleton pattern--------------------------------------------------
    private static volatile AdministradorDeBancoDados instance = null;

    // constructor
    private AdministradorDeBancoDados() {
        try {
            youtube = new URL("https://www.youtube.com/watch?v=gMW0Yxl-xXg");
            wikipedia = new URL("https://pt.wikipedia.org/wiki/Administrador_de_banco_de_dados");
            google = new URL("https://www.google.com.br/webhp?sourceid=chrome-instant&ion=1&espv=2&ie=UTF" +
                    "-8#q=%22Administrador+de+banco+de+dados%22");
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
    }

    public synchronized static AdministradorDeBancoDados getInstance() {
        if(instance == null) {
            instance = new AdministradorDeBancoDados();
        }
        return instance;
    }
