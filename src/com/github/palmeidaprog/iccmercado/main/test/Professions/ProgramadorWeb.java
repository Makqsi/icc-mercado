...
public class ProgramadorWeb implements Professionable, Linkable {
    // Links
    private URL youtube;
    ...
    private final String PROFESSION = "Programador Web";


    //--Singleton pattern--------------------------------------------------
    private static volatile ProgramadorWeb instance = null;

    // constructor
    private ProgramadorWeb() {
        try {
            youtube = new URL("https://www.youtube.com/watch?v=GEfuOMzRgXo");
            wikipedia = new URL("https://pt.wikipedia.org/wiki/Desenvolvimento_web");
            google = new URL("https://www.google.com.br/webhp?sourceid=chrome-instant&ion=1&espv=2&i" +
                    "e=UTF-8#q=%22programador%20web%22");
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
    }

    public synchronized static ProgramadorWeb getInstance() {
        if(instance == null) {
            instance = new ProgramadorWeb();
        }
        return instance;
    }
    ...
    public Scene getDetails() {
        FXMLLoader loaderResult = new FXMLLoader(getClass().getResource("programador_web.fxml"));
        loaderResult.setController(ProgramadorWebController.getInstance());
        Parent root = null;
        try {
